import { loadData, setSalary, addExpense as dataAddExpense } from './js/data.js';
import { calculateAvailableBalance } from './js/calculations.js';
import {
    updateAllDisplays,
    clearTransactionForm,
    getTodayDateString,
    showNotification,
    updateSalaryDisplay,
    updateCategoryTotalsDisplay,
    updateSavingsBalanceDisplay,
    updateDistributionStatus
} from './js/ui.js';
import { updateCharts } from './js/charts.js';
import { initializeNavigation, switchView } from './js/navigation.js';

// --- Elementos Globais ---
const salaryInput = document.getElementById('salary');
const transactionForm = document.getElementById('transactionForm');
const addTransactionBtn = document.getElementById('addTransactionBtn');
const modal = document.getElementById('transactionModal');
const modalCloseBtn = document.getElementsByClassName('close')[0];

// --- Funções de Handler ---
function handleSalaryChange(event) {
    setSalary(event.target.value);
    // Atualiza apenas os displays afetados diretamente + gráficos
    updateSalaryDisplay();
    updateSavingsBalanceDisplay(); // Saldo disponível pode depender do salário se a regra mudar
    updateDistributionStatus(); // Status da regra depende do salário
    updateCharts();
}

function handleAddTransaction(event) {
    event.preventDefault();

    const amountInput = document.getElementById('amount');
    const categoryInput = document.getElementById('category');
    const descriptionInput = document.getElementById('description');
    const dateInput = document.getElementById('date');

    const amount = parseFloat(amountInput.value);
    const category = categoryInput.value;
    const description = descriptionInput.value.trim();
    const date = dateInput.value;

    // Validações básicas
    if (!date || !category || !description || isNaN(amount) || amount <= 0) {
        if (!date) showNotification('Por favor, selecione uma data.', 'error');
        else if (!category) showNotification('Por favor, selecione uma categoria.', 'error');
        else if (!description) showNotification('Por favor, insira uma descrição.', 'error');
        else showNotification('Por favor, insira um valor válido.', 'error');
        return;
    }

    // Verifica saldo antes de adicionar Needs/Wants
    if (category === 'needs' || category === 'wants') {
        const currentAvailableBalance = calculateAvailableBalance();
        if (amount > currentAvailableBalance) {
            showNotification(`Saldo disponível (R$${currentAvailableBalance.toFixed(2)}) insuficiente para esta despesa de R$${amount.toFixed(2)}.`, 'error', 5000);
            return; // Bloqueia
        }
    }

    // Adiciona a despesa usando a função do módulo data
    dataAddExpense(amount, category, description, date);

    // Atualiza toda a UI relevante após adicionar
    updateAllDisplays(); // Atualiza lista, totais, saldo, status
    updateCharts(); // Atualiza gráficos

    showNotification(`Transação "${description}" adicionada.`, 'success', 2000);
    closeModal();
}

function openModal() {
    document.getElementById('date').value = getTodayDateString();
    modal.style.display = 'flex';
}

function closeModal() {
    modal.style.display = 'none';
    clearTransactionForm();
}

// --- Inicialização ---
function initializeApp() {
    loadData();
    updateAllDisplays(); // Atualiza toda a UI inicial
    initializeNavigation();
    switchView('transactions-view'); // Define a view inicial

    // Configura Event Listeners
    salaryInput.addEventListener('change', handleSalaryChange);
    transactionForm.addEventListener('submit', handleAddTransaction);
    addTransactionBtn.addEventListener('click', openModal);
    modalCloseBtn.addEventListener('click', closeModal);

    // Fechar modal clicando fora
    window.onclick = function(event) {
        if (event.target == modal) {
            closeModal();
        }
    }

    // Registro do Service Worker
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.register('/sw.js')
            .then(reg => console.log('Service Worker registrado:', reg))
            .catch(err => console.log('Falha ao registrar Service Worker:', err));
    }
}

// --- Ponto de Entrada ---
document.addEventListener('DOMContentLoaded', initializeApp);
